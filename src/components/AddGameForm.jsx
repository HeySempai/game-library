...
const VICTORY_TYPES = [
  { value: "score_descending", label: "Puntaje (Mayor gana)", emoji: "📊" },
  { value: "score_ascending", label: "Puntaje (Menor gana)", emoji: "📉" },
  { value: "absolute_winner", label: "Ganador absoluto", emoji: "🏆" },
  { value: "team_winner", label: "Por equipos", emoji: "👥" },
  { value: "cooperative", label: "Cooperativo", emoji: "🤝" },
  { value: "no_winner", label: "Sin ganador", emoji: "🎉" },
];
...
export default function AddGameForm({ games, players, preloadOwner, onAdd, onClose }) {
  ...
  const [victoryType, setVictoryType] = useState("absolute_winner");
  const [imageUrl, setImageUrl] = useState("");
  const [uploading, setUploading] = useState(false);

  const handleImageUpload = async (e) => {
    const file = e.target.files?.[0];
    if (!file || !form.nombre.trim()) return;
    setUploading(true);
    try {
      const id = form.nombre.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/-+$/, "");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              {["Juego Base", "Ampliacion", "Expansion"].map((t) => (
                <button key={t} type="button" onClick={() => setForm({ ...form, tipo: t })}
                  className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors cursor-pointer ${
                    form.tipo === t ? "bg-orange-500 text-white" : "bg-gray-100 text-gray-500 hover:bg-gray-200"
                  }`}>{t}</button>
              ))}
            </div>
          </div>
          ...
          {isExpOrAmp && (
            <div>
              <label className="block text-sm font-medium text-gray-500 mb-1">Juego Base</label>
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              {imageUrl ? (
                <img src={imageUrl} alt="" className="w-16 h-20 rounded-lg object-contain bg-gray-50" />
              ) : (
              ...
              <label className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors cursor-pointer ${
                ...
                {uploading ? (<><Loader2 size={14} className="animate-spin" /> Subiendo...</>) : (<><Upload size={14} /> Subir portada</>)}
                <input type="file" accept="image/*" onChange={handleImageUpload} disabled={uploading || !form.nombre.trim()} className="hidden" />
              </label>
            </div>
            {!form.nombre.trim() && <p className="text-[10px] text-gray-300 mt-1">Escribe el nombre primero</p>}
          </div>
          ...
          {form.tipo !== "Expansion" && (
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" min="1" value={form.minJugadores}
                  onChange={(e) => setForm({ ...form, minJugadores: e.target.value })}
                  className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-700 text-sm focus:border-orange-400 focus:outline-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-500 mb-1">Max Jugadores</label>
                <input type="number" min="1" value={form.maxJugadores}
                  onChange={(e) => setForm({ ...form, maxJugadores: e.target.value })}
                  className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-700 text-sm focus:border-orange-400 focus:outline-none" />
              </div>
            </div>
          )}
          ...
          {/* Developer */}
          <div>
            <label className="block text-sm font-medium text-gray-500 mb-1">Developer</label>
          ...
          {/* Victory Type */}
          {form.tipo === "Juego Base" && (
            <div>
              <label className="block text-sm font-medium text-gray-500 mb-2">Lógica de Victoria</label>
              <div className="grid grid-cols-2 gap-2">
                {VICTORY_TYPES.map((vt) => (
                  <button key={vt.value} type="button" onClick={() => setVictoryType(vt.value)}
                    className={`text-xs px-3 py-2 rounded-xl border transition-colors cursor-pointer text-left ${
                      victoryType === vt.value
                        ? "bg-orange-500 border-orange-400 text-white"
                        : "bg-gray-50 border-gray-200 text-gray-600 hover:border-gray-300"
                    }`}>
                    <span className="mr-1">{vt.emoji}</span> {vt.label}
                  </button>
                ))}
              </div>
            </div>
                ...
                {allCategories.map((cat) => (
                  <button key={cat} type="button" onClick={() => setCategory(category === cat ? "" : cat)}
                    className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-colors cursor-pointer ${
                      category === cat ? "bg-orange-500 text-white" : "bg-gray-100 text-gray-500 hover:bg-gray-200"
                    }`}>{cat}</button>
                ))}
              </div>
            </div>
          ...
          {/* Owners */}
          <div>
            <label className="block text-sm font-medium text-gray-500 mb-1">Owners</label>
            <div className="flex flex-wrap gap-2 mb-2">
              {players.map((p) => (
                <button key={p} type="button" onClick={() => toggleOwner(p)}
                  className={`text-xs px-3 py-1.5 rounded-full border transition-colors cursor-pointer ${
                    form.owners.includes(p) ? "bg-orange-500 border-orange-400 text-white" : "bg-gray-50 border-gray-200 text-gray-500 hover:border-gray-300"
                  }`}>{p}</button>
              ))}
            </div>
            {form.owners.filter((o) => !players.includes(o)).map((o) => (
              <span key={o} className="inline-flex items-center gap-1 text-xs bg-orange-100 text-orange-600 px-2 py-1 rounded-full mr-1 mb-1">
                {o} <button type="button" onClick={() => removeOwner(o)} className="cursor-pointer"><Trash2 size={10} /></button>
              </span>
            ))}
            <div className="flex gap-2 mt-1">
              ...
              <button type="button" onClick={addOwner} className="p-1.5 bg-orange-500 rounded-lg hover:bg-orange-400 text-white cursor-pointer">
                <Plus size={14} />
              </button>
            </div>
          </div>
